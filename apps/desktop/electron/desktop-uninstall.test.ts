...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  ...
  assert.deepEqual(uninstallArgsForMode('data'), ['-m', 'hermes_cli.uninstall', '--mode', 'data'])
})
...
test('UNINSTALL_MODES lists exactly the four supported modes', () => {
  assert.deepEqual([...UNINSTALL_MODES].sort(), ['data', 'full', 'gui', 'lite'])
})
